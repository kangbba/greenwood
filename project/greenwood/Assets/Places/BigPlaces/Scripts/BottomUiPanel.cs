...
{
    [SerializeField] private Image _bottomUiPanelBackground;
    ...
    private void Awake()
    {
        ...
        // ✅ UI가 처음에는 꺼진 상태로 시작
        _bottomUiPanelBackground.gameObject.SetAnim(false, 0f);
    }
    ...
    private void AddButton(BottomUiButtonType buttonType, System.Action onClickAction)
    {
        ...
        Button newButton = Instantiate(prefab, _bottomUiPanelBackground.transform);
    ...
    {
        bool hasButtons = _activeButtons.Count > 0;
        _bottomUiPanelBackground.gameObject.SetAnim(hasButtons, 1f);
        ...
        float startX = _bottomUiPanelBackground.rectTransform.rect.width / 2 - totalWidth; // 오른쪽 끝에서 시작
